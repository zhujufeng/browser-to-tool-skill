...
def main():
    ...
    check('待运行场景完整且不冒充已通过', {c['id'] for c in cases['evals']} == set(range(1, 30)) and '尚无模型通过结果' in cases['status'])
    ...
    check('路由文本：扩展网络能力不由端口判断', all(word in browser for word in ('Developer mode', '9222', 'chrome.debugger', '模型返回前的本地过滤')))
    check('路由文本：拒绝不通过换工具绕过', '政策拒绝不是技术缺口' in browser and '不能靠换工具继续被拒访问' in browser)
    check('路由文本：空监听与借用生命周期', all(word in browser for word in ('空监听先诊断', '不重启个人Chrome', '自建无登录环境验证', '逐项恢复并核对')))
    check('策略文本：区分本地分页及业务导出', all(word in http for word in ('页面只做本地分页', '不猜不存在的page参数', '全量前先核对业务样例与固定列', '合法缺值')))
    verification = (SKILL / 'references/verification.md').read_text(encoding='utf-8')
    check('验收文本：新版本新结果与复用记录', all(word in verification for word in ('本轮runId', 'manifest版本', '不为截图或收尾再采一遍', '生产导出代码')))
    check('交付文本：从入口到可定位结果', all(word in text for word in ('用户能打开入口', '本次文件的绝对路径', '待验证版本')) and all(word in verification for word in ('无关工作目录', '开始采集.command', '开始采集.cmd', '同秒冲突')))
    check('数据文本：详情模式和必需字段实测', '列头存在不等于字段已采到' in text and '必需字段未取得就是未完成' in verification and '已有站点库/官方API' in http)
    check('执行文本：授权后继续与真实导出门槛', all(word in text for word in ('不是结束条件', '不等用户再催', '列表 → 必需详情内容 → 正式导出文件 → 重新读取核对')))
    check('详情文本：逐项取数、关联及逐字段恢复', all(word in text for word in ('字幕轨道不等于正文', '去重形成待处理清单', '接口优先不是接口限定')) and all(word in verification for word in ('来源确认不存在', '失败待补', '只补待处理字段', '部分预览', '两个不同ID')) and '列表HTTP＋详情DOM' in http)
    check('文本保真：语言、合并及缓存', all(word in text for word in ('未要求翻译就不翻译', '不让详情默认英文标题覆盖', '页面、采集值和实际导出')) and all(word in verification for word in ('原本纯英文', '中英混合', '逐值核对导出')) and all(word in http for word in ('语言及来源口径属于恢复条件', '设置参数只是请求意图')))
    check('排错文本：分层诊断与回退覆盖', all(word in http for word in ('Requested format is not available', 'ignore_no_formats_error', '视频编辑权限', '当前助手浏览器只能逐页操作')))
    setup = (SKILL / 'references/setup.md').read_text(encoding='utf-8')
    check('环境文本：实际入口与TLS诊断', all(word in setup for word in ('启动文件实际调用的解释器', 'CERTIFICATE_VERIFY_FAILED', '企业CA由IT提供', '保持TLS校验', '修复代码已准备，联网待验证')))
    ...
    check('入口分流：先识别客户端运行形态与工具', '客户端＋运行形态＋真实工具' in text and all(word in clients for word in ('Codex终端 / IDE', 'Claude Code终端 / IDE', 'SSH / 容器 / WSL / 云端', 'localhost指向用户电脑')))
    check('入口分流：不向其他宿主套桌面设置', '不让其他客户端寻找该设置' in text and '不走上一节的桌面设置流程' in clients and '默认没有' in clients)
    runtime = (SKILL / 'references/playwright-runtime.md').read_text(encoding='utf-8')
    check('运行器文档：保留标签、原地截图与嵌套证据', all(word in runtime for word in ('protocolVersion:2', 'select-page', 'screenshot', 'inspect-request', 'rootFields', 'blocks.0.page')))
    readme = (ROOT / 'README.md').read_text(encoding='utf-8')
    check('入口一致性：分页例外同步', all('本地分页、单页或零条' in doc for doc in (readme, browser, clients, runtime)))
    check('入口一致性：运行器命令不冒充通用API', '仅使用配套运行器时采用`pages`、`select-page`、`screenshot`' in text)
