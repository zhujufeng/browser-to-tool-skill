...
开发与验证时可导入`<Skill根目录>/scripts/browser.mjs`的`startBrowser(options)`、`callBrowser({profile,session,command,output})`。成品的正常运行不应要求用户打开编码助手或手工调用其工具。优先按已验证的固定查询制作插件或独立脚本；只有确实需要复用本运行器时，才将必要运行模块、依赖和启动流程作为明确的本地依赖交付，并验证新进程启动，不依赖助手安装目录或开发期仍存活的worker。不把开发者个人绝对路径写进可分享源码，不复制登录profile/凭据到交付目录；依赖准备由助手协助完成，不让同事找接口。
