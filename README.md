...
**不绑定 Codex 自带浏览器。** 已有合规工具够用就复用；否则可用随附的 Playwright 专用浏览器脚本，首次自行登录，后续复用登录资料。依赖按需获准准备，不需要另搭 MCP，也不复制个人 Chrome。
...
先按上面说明安装或更新完整 Skill，已有版本先备份，并新开对话加载。不要复制这台电脑的 profile、Cookie、`node_modules`或浏览器缓存；新 Mac 首次需要在专用窗口重新登录。

把下面这段话发给助手：

> 请使用最新 browser-to-tool 的配套 Playwright 脚本，为 `<后台网址>` 做接口采集验证。先创建桌面任务目录、检查环境，缺依赖时说明用途并等我同意再准备。打开专用浏览器由我登录，不接管个人 Chrome。先监听真实查询，验证 HTTP 一页和下一页，不先点下载，不直接全量。过程和结果留在同一任务目录，结束后正常停止浏览器。
...
| 只有截图、点击工具 | 宿主允许时使用配套 Playwright 脚本；缺依赖先说明并获准，不默认退回下载按钮 |
...
- 启动专用可见 Chromium，跨命令保持会话；关闭、重开后复用可持久化登录资料。
- 先监听 Fetch/XHR，再通过必要的界面查询取得候选，用共享 Cookie 的 HTTP 请求验证下一页。
- 默认只返回结构；经授权的字段样本在本地过滤后输出到桌面任务，文件不覆盖。
